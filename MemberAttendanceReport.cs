...
namespace library_zone
{
    public partial class MemberAttendanceReport : Form
    {
        ...
        private void btn_login_Click(object sender, EventArgs e)
        {
            ...
            MySqlDataAdapter sda1 = new MySqlDataAdapter("select count(*) from branchadminlogin where UserName='" + tb_username.Text + "' and 	Password='" + tb_pwd.Text + "'", con);
            DataTable dt1 = new DataTable();
            sda1.Fill(dt1);
            if (dt1.Rows[0][0].ToString() == "1")
            {
                    MySqlDataAdapter sda = new MySqlDataAdapter("select * from memberattendance where branchId IN(select branchId from employee where UserName='" + tb_username.Text + "')", con);
